    ...
    def _build_graph(self):
        ...
        def softmax_layer(input):
            ...
            with tf.variable_scope('softmax', reuse=tf.AUTO_REUSE):
                P = tf.get_variable('P', (self.vec_dim), dtype=tf.float32,
                                    initializer=tf.random_uniform_initializer(-1., 1.))
                b = tf.get_variable('b', (), dtype=tf.float32, initializer=tf.zeros_initializer())
                self.P = P
                self.b = b
            return tf.reduce_sum(input * P, axis=1) + b
        ...
        self.classification_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(self.train_y, tf.float32), logits=train_logits))

        # compute full loss
        self.loss = (1 - self.alpha) * self.classification_loss + self.alpha * self.proj_loss

        # compute accuracy counts
        self.pred_train = tf.cast(train_logits >= 0, tf.int32)

        # predict test labels:
        test_logits = softmax_layer(project_target(self.test_x))
        self.pred_test = tf.cast(test_logits >= 0, tf.int32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path',
                        ...
                        default='./checkpoints/lrblse.bin')
